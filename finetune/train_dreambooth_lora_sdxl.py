...
# Modifications by Zahra TehraniNasab (2025): adapted for CheXpert training, SDXL LoRA fine-tuning, and HPC execution.
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The root path to the images for CheXpert dataset. If not provided, the script will download the dataset"
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The path to the CSV file containing the training data for CheXpert dataset. The CSV file should contain"
            " the image file names and the labels."
        ),
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        help=(
            "Whether to run the script in debug mode. In this mode, the script will only train for 2 steps in each epoch."
        ),
    )
...
class CheXpertDataset(Dataset):
    ...
    def create_captions(self):
        ...
        for _, row in self.df.iterrows():
            findings = [c for c in conditions if row.get(c, 0) == 1]
    ...
    def __getitem__(self, idx):
        ...
        image_path = os.path.join(self.image_root_path, row['Path']).replace("CheXpert-v1.0/", f"CheXpert-v1.0_{self.size}x{self.size}/")
        caption = row['caption']
        ...
        return {
            ...
            "crop_top_left": (0, 0)  # Assuming no crop info
        }

def collate_fn_chexpert(batch):
    pixel_values = torch.stack([ex["pixel_values"] for ex in batch])
    prompts = [ex["prompt"] for ex in batch]
    original_sizes = [ex["original_size"] for ex in batch]
    crop_top_lefts = [ex["crop_top_left"] for ex in batch]
    return {
...
def main(args):
    ...
    unet_target_modules = [
        ...
        # Self-attention (attn1 blocks)
        "attn1.to_q", "attn1.to_k", "attn1.to_v", "attn1.to_out.0",

        # Feed-forward projections (optional)
        "ff.net.0.proj", "ff.net.2",  # MLP blocks

        # Optionally more modules if needed
    ]
    ...
    train_dataloader = torch.utils.data.DataLoader(
        ...
        shuffle=True,
        collate_fn=collate_fn_chexpert,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            if args.debug and step >= 2:
                logger.warning("Debug mode enabled, stopping training after 2 steps.")
                break
        ...
        save_dir = os.path.join(args.output_dir, f"lora_weights_epoch_{epoch}")
