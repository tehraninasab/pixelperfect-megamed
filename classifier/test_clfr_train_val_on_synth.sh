...
#SBATCH --job-name=px
#SBATCH --output=/home/a/amarkr1/workshop/megamed/logs/slurm-%j.out
#SBATCH --time=1:00:00
...
VAL_CSV="datasets/chexpert_highres_synthetic/synthesized_images_sdxl_chexpert_dreambooth_lora_rank8_without_text_interactive_epoch_3_0_-1_32_set1.csv"
TRAIN_CSV="datasets/chexpert_highres_synthetic/synthesized_images_sdxl_chexpert_dreambooth_lora_rank16_without_text_epoch_3_0_-1_32_set1.csv"
TEST_CSV="datasets/chexpert_highres/visualCheXbert_test.csv"
REAL_ROOT="/scratch/a/amarkr1/data"
SYNTHETIC_ROOT="."
BASE_DIR="classifier/results_train_synth_val_synth_reversed"
NUM_WORKERS=4
LEARNING_RATE=1e-4
NUM_EPOCHS=10
NUM_GPUS=4
...
run_training() {
    ...
    # Build command
    local cmd="CUDA_VISIBLE_DEVICES=${gpu_id} python /home/a/amarkr1/workshop/megamed/classifier/train_chexpert.py \
        --train_csv ${TRAIN_CSV} \
...
for size in 1024; do
    for model in "efficientnet-b0" "resnet50"; do
        for pretrained in true false; do
            echo "Preparing configuration for size: $size, model: $model, pretrained: $pretrained"
